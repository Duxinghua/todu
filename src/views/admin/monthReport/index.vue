  ...
  <div class="self-container">
    <div class="self-box">
      <div class="self-box2">
        ...
        <div style="padding-left: 30px;font-weight: bold">周报统计</div>
      </div>
      <div class="self-box3">
        ...
        <div class="search-row">
          ...
          <el-input v-model="form.searchProject" placeholder="请输入项目" :clearable="true" />
        </div>
        <div class="search-row">
      ...
      <div v-else class="self-box3" style="flex: 1;flex-wrap: wrap;">
        <el-button type="primary" :disabled="exportDisabled" @click="exportData">导出</el-button>
          ...
          <div>
            <el-row :gutter="20" style="display: flex;margin-bottom: 20px">
              <el-col :span="12" class="self-input-box" style="margin-right: 10px">
                ...
                <el-input v-model="dailyForm.projectName" :readonly="readonly" size="small" class="self-input" />
              </el-col>
              <el-col :span="12" class="self-input-box" style="margin-left: 10px">
                <div class="self-title">项目编码</div>
                <el-input v-model="dailyForm.proCode" :readonly="readonly" size="small" class="self-input" />
              </el-col>
            </el-row>
            ...
            <el-row style="margin-top: 10px">
              <h3>存在问题</h3>
              <div style="width: 100%;">
                <div class="self-content" v-html="dailyForm.problem" />
              </div>
            </el-row>
            <el-row style="margin-top: 10px">
              <h3>下步计划 / 工作重点提示</h3>
              <div style="width: 100%;">
                <div class="self-content" v-html="dailyForm.keyPoint" />
              </div>
            </el-row>
          </div>
        </div>
...
<script>
import { weeklyPersonList, weeklyDelete, weeklyBatchDelete } from '@/api/sMonthReport'
import { reportView } from '@/api/monthReport'
import { projectList } from '@/api/dayReportList'
export default {
  name: 'DayPersonList',
  data() {
    return {
  ...
  methods: {
    ...
    showReport(row) {
      reportView({ id: row.id }).then(res => {
        ...
        if (status === 200) {
          ...
          data.projectName = row.proName
          data.proCode = row.proCode
          this.dailyForm = data
        }
      })
      this.drawer = true
    ...
    deleteAllRow() {
      this.$confirm('是否确认删除数据', '提示', {
        ...
        weeklyBatchDelete(ids).then(res => {
          const { status, msg } = res
          if (status === 200) {
            this.getList()
            this.$message.success('删除成功')
    ...
    exportData() {
      ...
      ids = ids.substr(0, ids.length - 1)
      this.$router.push({ path: '/sMonthReport/totalReport', query: {
        ids: ids
      }})
    },
    ...
    getList() {
      console.log(this.searchDateLine)
      // 需要添加条件
      let date1 = ''
      let date2 = ''
      if (this.searchDateLine !== null && this.searchDateLine.length > 1) {
