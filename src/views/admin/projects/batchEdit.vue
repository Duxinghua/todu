<template>
  ...
  <div class="self-container">
    <div class="self-box">
      ...
      <div slot="header">
        <span>批量添加(<span style="font-size: 14px;">工号，姓名为必填项</span><span style="color: red;">*</span>)</span>
      </div>
      <div style="flex: 1">
        <!-- 数据表格 -->
        <el-table
        ...
        >
          ...
          >
            <template slot-scope="scope">
              <el-input v-model="scope.row.workNumber" placeholder="请输入工号" autocomplete="off" size="mini" @blur.stop="validateWorkNum(scope.row)" />
            </template>
          </el-table-column>
          ...
          <el-table-column label="所属室所" align="center">
            <template slot-scope="scope">
              <el-select
                ref="deptSelect"
...
<script>
import { batchSavePerson, deptList, checkWorkNumber } from '@/api/dept'
...
export default {
  ...
  methods: {
    ...
    validateWorkNum(row) {
      if (row.workNumber !== '') {
        checkWorkNumber({ workNumber: row.workNumber }).then(res => {
          if (res.data) {
            row.workNumber = ''
    ...
    formatterRow(row) {
      ...
      if (majorId) {
        ...
        row.userMajor = majorObj.deptName
      }
      // if (posId) {
