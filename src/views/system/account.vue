    ...
    <div class="self-box">
      ...
      <el-row>
        <el-col>
          ...
          <el-input v-model="queryUserName" placeholder="请输入用户名关键字" style="width: 200px" size="mini" :clearable="true" />
          <span style="font-size: 12px;margin: 20px 0 0 10px">工号</span>
          <!--          <el-input v-model="queryWorkNumber" placeholder="请输入工号关键字" style="width: 200px" size="mini" :clearable="true" />-->
          <!--          <span style="font-size: 12px;margin: 20px 0 0 10px">手机号</span>-->
          <el-input v-model="queryContactInfo" placeholder="请输入手机号关键字" style="width: 200px" size="mini" :clearable="true" />
...
export default {
  ...
  methods: {
    ...
    validateWorkNum(val) {
      if (this.userForm.workNumber !== '' && this.userForm.workNumber !== undefined) {
    ...
    doExportData() {
      ...
      import('@/vendor/Export2Excel').then(excel => {
        const filterVal = ['userName', 'workNumber', 'contactInfo', 'createTime']
        const tHeader = ['姓名', '工号', '手机号', '创建时间']
